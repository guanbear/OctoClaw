...
### 4.0.5 灰区任务：规则主判 + 语义复核

后续不应把所有任务都交给模型做语义判断，也不应退回到“纯关键词分类器”。
...
2. **只有灰区任务才触发语义复核**
   - 例如：
     - `runner` 和 `spawn_single` 分差很小
     - 文本既像“本机检查”又像“本机改造”
     - 同时带 `research + tools + implementation`
3. **语义复核只调用便宜且快的专用 router 模型**
   - 返回严格 JSON
   - 不做长推理，不读大上下文
   - 目标只是输出：
     - `task_kind`
...
- 大多数请求仍然零额外 token
- 只有灰区任务才花一笔很小的语义判断成本
- 最终 route 仍然由 OctoClaw 自己综合规则、语义、运行时状态决定

### 4.0.6 速度优先的 router 模型

灰区任务如果需要额外语义判断，最重要的不是“最强模型”，而是：

- 足够快
- 足够便宜
- 结构化输出稳定

因此 `octopus-router` 应单独建模，不与 `main/analyze` 混用。
...
- `octopus-runner`
  - 优先最低延迟、最高吞吐的快模型
- `octopus-router`
  - 也优先快模型，因为它服务的是“路由判定延迟”
- `main/analyze/power`
  - 再优先更强的模型

这意味着：

- `runner` 和 `router` 的打分中，`ttft/output_tps` 权重应明显高于价格与通用榜单
- 如果本地实测 `MiniMax M2.7 highspeed` 在你环境里最快，它就应该成为 `octopus-router` 的首选
- 外部 benchmark 仍然重要，但在 `router` 这个角色里只做辅助，而不是主裁判
...
- **工具型观察任务** 不分本机还是远程，只要本质是“查状态 / 查版本 / 查端口 / 查日志 / 查资源”，都优先归 `runner`
- **工具型实施任务** 才进入 `spawn_single`

也就是说：

- `查 macmini 的 openclaw 版本`
- `检查远程主机上的 redis 端口和日志`
- `检查当前机器 python 版本、磁盘和内存`

这些都属于同一类执行形态：**观察型工具任务**

另外，“给我一句总结 / 三行总结”只是输出格式要求，不应该把观察任务误判成 `writer` 或 `spawn`。
