...
1. v1 里 `worker_code` 是否全量先上 `gpt-5.4`，还是提前拆成 normal/deep 两档。
2. Phase 1 的最小 status surface 是否只先做 `status / details / queue`，还是同时带 `timeline` 占位。
3. 何时开始把本地模型接入 `judge_fast` shadow lane。
