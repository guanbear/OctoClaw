...
Behavior:
  1. bootstrap isolated acceptance runtime
  2. if no session key/target is supplied, print the sessions path and stop
  3. otherwise run unified slack acceptance suite
EOF
}
...
if [ -z "${SESSION_KEY}" ] && [ -z "${TARGET}" ]; then
  cat <<EOF
[acceptance-live] bootstrap complete
