...
export function renderSlackAcceptanceMarkdown(report: SlackAcceptanceReport): string {
  ...
  lines.push("");
  lines.push("| Case | Status | ACK | Final | No Spawn | ACK ms | Final ms |");
  lines.push("|------|--------|-----|-------|----------|--------|----------|");
  for (const item of report.cases) {
    lines.push(`| ${item.kind} | ${item.status} | ${item.ack.status} | ${item.final.status} | ${item.noSpawn.status} | ${item.ackMs ?? "N/A"} | ${item.finalMs ?? "N/A"} |`);
  }
  lines.push("");
  for (const item of report.cases) {
