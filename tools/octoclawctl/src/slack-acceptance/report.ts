...
export function renderSlackAcceptanceMarkdown(report: SlackAcceptanceReport): string {
  ...
  for (const item of report.cases) {
    ...
    if (item.replayEvidence) {
      ...
      lines.push(`- finalDelivery: footerVia=${item.replayEvidence.footerVia ?? "N/A"} footerDifficulty=${item.replayEvidence.footerDifficulty ?? "N/A"} delivery_transport=${item.replayEvidence.deliveryTransport ?? "N/A"} target_source=${item.replayEvidence.targetSource ?? "N/A"} footer_source=${item.replayEvidence.footerSource ?? "N/A"} duplicateFinal=${item.replayEvidence.duplicateFinalCount ?? "N/A"} parentEchoAfterNativeAnnounce=${item.replayEvidence.parentEchoAfterNativeAnnounceCount ?? "N/A"}`);
