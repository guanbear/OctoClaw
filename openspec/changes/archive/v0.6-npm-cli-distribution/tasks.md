...
> ## Archive note
>
> Archived 2026-05-15. Package distribution shipped.
>
> **Shipped (verified):**
>
> - Package renamed to `@octoclaw/cli` with `bin: { octoclawctl, octoclaw }`.
> - `npm pack` artefact validated locally.
> - Init wizard in `tools/octoclawctl/src/commands/init/` with five steps
>   (openclaw-check / judge-model / im-token / im-channel / doctor-verify).
> - `octoclawctl init --help` documented; non-interactive mode supported.
>
> **Skipped (no environment):** Public npm publish from CI. Done manually on
> first release; not in scope for this change.
