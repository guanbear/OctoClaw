...
> ## Archive note
>
> Archived 2026-05-15. All five work-packets shipped; checkboxes left unticked
> on this file because the work was done in parallel slices and ticking was
> never backfilled. Verification below points at the live code.
>
> **Shipped (verified by `grep_search` and existing tests):**
>
...
> - `extensions/octoclaw-runtime/src/router-lite/shadow-bridge.ts` (with
>   `shadow-bridge.test.ts` covering missing snapshot, low confidence, status
>   request, selector throws, writer throws)
> - Call site at `extensions/octoclaw-runtime/src/resolve/policy-resolver.ts`
>   line 1762 (`emitRouterLiteShadowEvent(...)`).
> - `octoclawctl router shadow-report [--input <jsonl>] [--format json]`
>   sub-command in `tools/octoclawctl/src/cli.ts`.
>
> **Skipped (no environment):** WP-6 7-day shadow observation loop and
> live-Slack smoke acceptance — local CI has no Slack workspace to drive these.
> Tracked instead under `v0.6-stability-hardening` Slack smoke phase.
>
> **Hard invariants** (unchanged): live route still `reply | delegate` only;
> shadow failures (snapshot missing / IO / parse) are silently dropped; live
