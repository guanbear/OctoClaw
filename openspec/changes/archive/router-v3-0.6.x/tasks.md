...
> ## Archive note
>
> Archived 2026-05-15. Phase A + Phase B + Phase C-implementation shipped.
> Remaining items moved to `parking/router-v3-wizard-and-release/` because they
> need real install traffic to validate (interactive 7-step wizard UX, plan
> quota auto-poll, end-to-end release smoke with 30+ shadow samples).
>
> **Shipped (verified by reading the source tree and CLI):**
>
> - **Phase A — package extraction**: `packages/octoclaw-router/` exists with
>   11 sub-modules (capability / cost / decision / health / overrides /
>   promotion / scoring / semantic / wizard / invariants / data). Judge
>   migrated into `semantic/`, schema reduced to 3 fields (`route /
>   confidence / complexity`), see `semantic/judge-schema.ts`.
> - **Phase B — capability + scoring**: `data/leaderboard-snapshot.json`,
>   `scripts/refresh-leaderboard-snapshot.mjs`, `capability/merge.ts` with
>   conflict detection, `cost/index.ts` with SQLite-backed cost event store,
>   `scoring/index.ts` with the balanced 35/20/20/15/10 formula, deterministic.
> - **Phase C — shadow + promotion**: `decision/shadow-selector.ts`,
>   `promotion/index.ts` with `evaluatePromotion()`,
>   `evaluatePromotionForConfiguredModel()`, `parsePromotionDecisionLog()`,
>   `runLightweightPromotionReview()`, plus `octoclawctl router promotion
>   review` and `router decisions` CLI subcommands.
> - **Phase D — partial**: `wizard/index.ts` non-interactive wizard config
>   produced from `octoclawctl router wizard` step; cost report via
>   `generateCostReport(...)` and `octoclawctl router cost report`; override
>   CLI (`router score override / reset`, `router model mark / ban /
>   list-overrides`).
>
> **Deferred to `parking/router-v3-wizard-and-release/`:**
>
> - Phase D1 full 7-step interactive wizard with provider plan auto-detect.
> - Phase D6 plan quota poll + < 10% auto-switch to non-plan models.
> - Phase E1 end-to-end smoke (30+ delegate turns triggering auto-promotion).
> - Phase E2/E3/E4 release polish and version bump (handled in
>   `v0.6-stability-hardening` and the v0.6.0 release process directly).
>
> **Hard invariants** (unchanged): judge stays 3 fields; no keyword routing;
> unconfigured models never live; sub-agent auto-switch only, main agent never
> silently switched; shadow failure never affects live route.
