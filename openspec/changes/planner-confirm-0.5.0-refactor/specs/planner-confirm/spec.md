...
- WHEN a budgeted main fast path exceeds a fixed 30s soft main execution budget, needs write/long-running work, exceeds one late real read-only tool after timeout, or exceeds context budget
...
- AND a prompt-injection boundary after expiry SHALL inject a soft-budget notice rather than forcing immediate delegate
- AND one lightweight read-only tool after expiry MAY be allowed to finish the answer
- AND write tools, long commands, multi-step tools, tests/build/review/validation, or a second real read-only tool SHALL be blocked or rewritten to require `octoclaw_dispatch`
