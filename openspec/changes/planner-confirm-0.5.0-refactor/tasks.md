...
- [x] SR-P1 `budgeted_main_then_delegate` uses a fixed 30s soft main execution budget: expiry records `budgeted_main_escalated_pending`, late final records `budgeted_main_completed_late` without spawn, prompt-injection boundaries only add a soft-budget notice, one late lightweight read-only tool can finish the answer, and write/long/multi-step or second real read-only tools escalate through `octoclaw_dispatch` with replay metrics.
...
- [x] SR-P2 0.5.1 scheme-B feature-flag slice exists: speculative standby `sessions_spawn(mode="session")` hint injection, matching-only speculative spawn whitelist, `octoclaw_dispatch` `dispatchMode=send_to_speculative`, pending-intent-gated `sessions_send`, and unchanged confirm/ACK semantics. Live latency validation remains pending before default enablement.
...
- Current deployed release smoke after merge and commit `6d71bee`: `/tmp/octoclaw-release-smoke-20260504-post-6d71bee/out/slack-acceptance-2026-05-04-14-22-52.{json,md}` passed. Thread `1777904347.792319`, WorkContract `wc-2a33bfe882d50bee`, spawnIntent `nsp_moraehqa_52fff086`, runId `b911703f-3fd2-42ed-8d60-f00055ec34db`, childSession `agent:main:subagent:cb3f4298-1865-4640-9fab-79f4e47942c3`, decision_bucket `must_delegate`.
- Current deployed release report fields: neutralAckMs `4962` via Slack reaction, `anchor_source=ctx`, `fallback_used=false`; acceptedAckMs `129358`; finalMs `222613`; `delivery_transport=slack_api`, `target_source=inbound_anchor`, `footer_source=envelope`, `footerVia=native_announce`, `completion_file_timeout=0`, and `duplicateFinal=0`. StageMs: message_received `3610`, before_dispatch `3658`, before_model_resolve `16740`, before_prompt_build `86673`, octoclaw_dispatch `97829`, sessions_spawn_intent_allowed `117507`, sessions_spawn_accepted `129901`, dispatch_confirm `129904`, native_child_final `223250`.
- Current deployed SQLite row for `nsp_moraehqa_52fff086` is `accepted` with non-empty runId/childSessionKey and revision `3`.
