...
function parseOpenRouterRankings(html, catalogRecords) {
  ...
  return {
    ...
    counts: {
      ...
      retained_count: rankings.length,
...
function freshnessState(updatedAt, { freshHours = 24, staleHours = 96 } = {}) {
...
function buildSourceStatus({ source, primaryFile, lastGoodFile, error = "" }) {
  const primary = loadJson(primaryFile);
  const lastGood = loadJson(lastGoodFile);
  const usingLastGood = !primary && Boolean(lastGood);
  const payload = primary || lastGood || {};
  const updatedAt = String(payload?.generated_at || "").trim();
  return {
    source,
    primary_file: primaryFile,
    last_good_file: lastGoodFile,
    primary_present: Boolean(primary),
    last_good_present: Boolean(lastGood),
    using_last_good: usingLastGood,
    updated_at: updatedAt,
    freshness: freshnessState(updatedAt),
    records: Array.isArray(payload?.records) ? payload.records.length : 0,
    error: String(error || "").trim(),
  };
}

function renderCron({ workspace = WORKSPACE, intervalHours = DEFAULT_REFRESH_INTERVAL_HOURS } = {}) {
  const hours = Math.max(1, Number(intervalHours || DEFAULT_REFRESH_INTERVAL_HOURS));
  const script = path.resolve(path.dirname(new URL(import.meta.url).pathname), "model-intel-sync.mjs");
  const command = `WORKSPACE=${JSON.stringify(workspace)} node ${JSON.stringify(script)} refresh`;
  return {
    schema_version: "octoclaw.model_intel.refresh_schedule/v1",
    workspace,
    interval_hours: hours,
    cron: `0 */${hours} * * * ${command}`,
    command,
  };
}
...
async function refreshOpenRouterRankings(catalogRecords) {
  ...
  const rankingPayload = parseOpenRouterRankings(html, catalogRecords);
  const records = Array.isArray(rankingPayload.records) ? rankingPayload.records : [];
  const counts = rankingPayload.counts && typeof rankingPayload.counts === "object" ? rankingPayload.counts : {};
  const payload = {
    generated_at: nowIso(),
    ...
    filters: {
      ...
      policy: "paid_only_ecosystem_signal",
    },
    counts,
  };
...
async function main() {
  ...
  if (!["refresh", "status", "render-cron"].includes(command)) {
    process.stderr.write("Usage: model-intel-sync.mjs <refresh|status|render-cron>\n");
    process.exitCode = 2;
    return;
  }
    ...
    const payload = {
      ...
      sources: [
        buildSourceStatus({
          ...
          primaryFile: OPENROUTER_CATALOG_FILE,
          lastGoodFile: OPENROUTER_CATALOG_LAST_GOOD_FILE,
        }),
        buildSourceStatus({
          source: "openrouter_rankings",
          primaryFile: OPENROUTER_RANKINGS_FILE,
          lastGoodFile: OPENROUTER_RANKINGS_LAST_GOOD_FILE,
        }),
        buildSourceStatus({
          source: "models_dev_registry",
          primaryFile: MODELS_DEV_FILE,
          lastGoodFile: MODELS_DEV_LAST_GOOD_FILE,
        }),
      ],
    };
  ...
  if (command === "render-cron") {
    const hours = Number(options["interval-hours"] || DEFAULT_REFRESH_INTERVAL_HOURS);
    const workspace = normalizePath(options.workspace || WORKSPACE) || WORKSPACE;
    process.stdout.write(`${JSON.stringify(renderCron({ workspace, intervalHours: hours }), null, 2)}\n`);
    return;
  }
...
export const __modelIntelSyncTest = {
  buildSourceStatus,
  renderCron,
  freshnessState,
