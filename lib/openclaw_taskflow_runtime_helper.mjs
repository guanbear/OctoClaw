...
function usage() {
  console.error(
    "Usage: openclaw_taskflow_runtime_helper.mjs <action> [options]\n" +
      ...
      "  read-flow            --session-key <key> --flow-id <id>\n" +
      "  read-task            --session-key <key> --flow-id <id> --task-id <id>\n" +
...
async function readFlow(options) {
  if (!options.session_key || !options.flow_id) {
    throw new Error("Missing required read-flow arguments: session-key, flow-id");
  }
  const createPluginRuntime = await loadCreatePluginRuntime(options.openclaw_bin || "openclaw");
  const runtime = createPluginRuntime({});
  const bound = runtime.taskFlow.bindSession({ sessionKey: options.session_key });
  const flow = typeof bound.getFlow === "function" ? bound.getFlow({ flowId: options.flow_id }) : null;
  if (!flow) {
    return {
      ok: false,
      status: "not_found",
...
async function readTask(options) {
  if (!options.session_key || !options.flow_id || !options.task_id) {
  ...
  const bound = runtime.taskFlow.bindSession({ sessionKey: options.session_key });
  const flow = typeof bound.getFlow === "function" ? bound.getFlow({ flowId: options.flow_id }) : null;
  const task = Array.isArray(flow?.tasks)
    ? flow.tasks.find((entry) => String(entry?.taskId || "").trim() === String(options.task_id).trim()) || null
    : null;
  if (!task) {
    return {
      ok: false,
