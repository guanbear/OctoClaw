...
def render_detail_text(task: dict[str, Any], detail: dict[str, Any]) -> str:
    ...
    materialization_summary = str(materialization.get("summary", "") or "").strip()
    if materialization_summary:
        lines.append(f"Materialization: {materialization_summary}")
...
def render_retrieval_text(bundle: dict[str, Any]) -> str:
    ...
    if runner_plan:
        ...
        if summary:
            lines.append(f"Runner plan: {summary}")
    materialization = bundle.get("materialization", {}) if isinstance(bundle.get("materialization"), dict) else {}
    materialization_summary = str(materialization.get("summary", "") or "").strip()
    if materialization_summary:
        lines.append(f"Materialization: {materialization_summary}")
