...
class RuntimePolicyRolloutTests(unittest.TestCase):
    ...
    def test_recommend_blocks_unlinked_validation_summary(self) -> None:
        with tempfile.TemporaryDirectory(prefix="octoclaw-rollout-unlinked-") as tmpdir:
            config_path = Path(tmpdir) / "octopus-config.json"
            validation_path = Path(tmpdir) / "validation-summary.json"
            manifest_path = Path(tmpdir) / "feedback-manifest.json"
            config_path.write_text(
            ...
            manifest_path.write_text(
                json.dumps({"schema_version": "octoclaw.feedback_manifest/v1", "run_id": "feedback-a"}),
                encoding="utf-8",
            )
            validation_path.write_text(
                json.dumps(
