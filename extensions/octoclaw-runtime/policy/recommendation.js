...
export function buildRouteRecommendation(routeMeta = {}, resolved = {}) {
  ...
  const arbitrationRequired = Boolean(Number(features.repo_activity_hits || 0) > 0 || routeMeta?.needs_semantic_review);
  const strategy = Number(features.repo_activity_hits || 0) > 0
    ? "rule_fallback"
    : (routeMeta?.needs_semantic_review ? "route_hint_or_future_tiny_judge" : "none");
  const resolvedBy = Number(features.repo_activity_hits || 0) > 0 ? "rule_fallback" : "base_policy";
  return {
