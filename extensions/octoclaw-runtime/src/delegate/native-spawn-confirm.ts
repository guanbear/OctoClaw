import type { DelegateContract, NativeBindingRef, WorkContract } from "@octoclaw/contracts/work-contract";
...
function ensureDelegate(contract: WorkContract, nativeBinding: NativeBindingRef, intent: NativeSpawnIntent): DelegateContract {
...
function recordNativeRefs(input: {
  ...
  return updateWorkContract(input.intent.workContractId, (contract) => {
...
export async function confirmNativeSpawn(input: ConfirmNativeSpawnInput): Promise<ConfirmNativeSpawnOutput> {
  ...
  if (status !== "accepted") {
    ...
    return {
      ...
      error: failure.error || asString(input.error) || `sessions_spawn_${status || "not_accepted"}`,
      spawnIntentId,
      workContractId,
      runId: null,
      childRunId: null,
  ...
  if (!confirm.ok || !confirm.intent) {
    return { ok: false, status: confirm.status === "conflict" ? "conflict" : "error", error: confirm.error || "confirm_failed", spawnIntentId, workContractId, runId };
  }
