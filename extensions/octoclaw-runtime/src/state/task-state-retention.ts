...
function archiveReasonForTask(task: TaskStateRecord, nowMs: number, staleRetentionMs: number, terminalRetentionMs: number): string | null {
  ...
  const activeOrStaleStatuses = new Set(["running", "running_slow", "stalled", "queued", "planned", "materializing", "blocked", "timed_out", "registered", "deliverable_ready", "degraded", "lost", "sealed"]);
